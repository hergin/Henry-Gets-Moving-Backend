...
test.group('Diagrams', (group) => {
    ...
    test("can't create diagram without required fields", async ({ client, route }) => {
    ...
    test('create diagram', async ({ client, route }) => {
        ...
        const postResult = await client
            .post(route('DiagramsController.store'))
            .form(diagramToCreate)
    ...
    test('update diagram', async ({ client, route }) => {
        ...
        const result = await client
            .put(route('DiagramsController.update', { id: 1 }))
            .form(diagramToUpdate)
        result.assertStatus(200)
